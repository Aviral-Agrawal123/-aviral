...
public class BankServiceDaoImp implements BankServiceDao {
	...
	public Bank addCustomer(Bank bk) throws BankException {
		if(!valid.validateCustomer(bk.getName()))
		{
			try {
		...
		else if(!valid.validateMobile(bk.getMobile_no()))
		{
			try {
	...
	public Bank login(Bank bk, int accountno, String passwrd) throws BankException {
		if(bk.getPasswd().equals(passwrd))
		{
			return bk;
		}
